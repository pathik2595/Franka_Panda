...
Recommended source/assets to include:
...
The `assets/` folder now includes:

```text
assets/scene.usd
assets/panda.usd
assets/panda_description.yaml
assets/environment/
assets/objects/
```

`Trials/` contains the trial/debug folders from the successful run, so the README, screenshots, and LinkedIn post can reference the exact perception evidence.

Keep these out of Git unless you deliberately want large files or use Git LFS:

```text
.isaac_python_packages/
sam3.1_multiplex.pt
Recording/
isaac_bin_picking/perception_dataset/
isaac_bin_picking/perception_captures/
isaac_bin_picking/**/__pycache__/
*.pyc
*.npy
*.mp4
*.pt
...
Exception: `Trials/` is intentionally allowed in `.gitignore`, including its `.npy` point-cloud/debug files.

## Suggested GitHub Flow

From a clean clone or a new local Git repository:

```powershell
git init
git remote add origin https://github.com/pathik2595/Franka_Panda.git
git add README.md .gitignore Scripts assets Trials
git commit -m "Add Franka Panda Isaac Sim bin picking demo"
git branch -M main
git push -u origin main
```

If you want to publish videos, model checkpoints, or full perception datasets, use GitHub Releases, cloud storage, or Git LFS instead of regular Git commits.
